...
async function findAccount(accountId: number) {
...
export const transactionService = {
  deposit: async (accountId: number, data: DepositInput) => {
  ...
  withdraw: async (accountId: number, data: WithdrawalInput) => {
    const account = await findAccount(accountId);
    const cents = toCents(data.amount);
  ...
  transfer: async (accountId: number, data: TransferInput) => {
  ...
  balance: async (accountId: number) => {
    const account = await findAccount(accountId);

    return { balance: toReais(account.balance) };
  },

  statement: async (accountId: number) => {
    const account = await findAccount(accountId);
    ...
    const dataTransac = (list: { amount: number;[key: string]: unknown }[]) =>
      list.map(({ amount, ...rest }) => ({ ...rest, amount: toReais(amount) }));

    return {
      deposits: dataTransac(deposits),
      withdrawals: dataTransac(withdrawals),
      transfers_sent: dataTransac(sent),
      transfers_received: dataTransac(received),
    };
  },
};
